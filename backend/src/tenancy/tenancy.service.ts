...
import {
  StockDocument,
  StockDocumentLine,
} from '../inventory/entities/stock-document.entity';
import {
  SalesDocument,
  SalesDocumentLine,
...
import { Vehicle } from '../inventory/entities/vehicle.entity';
import { VehicleTrip } from '../inventory/entities/vehicle-trip.entity';
import { VehicleLocationHistory } from '../inventory/entities/vehicle-location.entity';
import {
...
export class TenancyService implements OnModuleDestroy {
  ...
  private async initializeTenantConnection(
    ...
    try {
      const company = await this.mainDataSource
        .getRepository(Company)
        .findOne({ where: { id: companyId } });

      if (!company) {
        throw new Error(
          `Empresa com ID "${companyId}" não encontrada na base de dados principal.`,
        );
      }
      ...
      console.log(
        `[Tenancy] Initializing for ${company.name} (${companyId}) -> DB: ${targetDbName}`,
      );

      const resolvedPort = Number(dbConfig.port || mainOptions.port || 5432);

      console.log(
        `[Tenancy] Connection setup for ${targetDbName}: Host=${dbConfig.host || mainOptions.host}, Port=${resolvedPort}, User=${dbConfig.username || mainOptions.username}`,
      );
      ...
      try {
        ...
        try {
          ...
          if (articlesTable && !articlesTable.findColumnByName('ivaCode')) {
            console.log(
              `[Tenancy] Patching 'articles' table in ${targetDbName}: Adding 'ivaCode' column`,
            );
            await queryRunner.query(
              'ALTER TABLE "articles" ADD COLUMN "ivaCode" character varying',
            );
          }
          ...
          if (taxTable) {
            ...
            if (
              idCol &&
              (idCol.type.toLowerCase().includes('uuid') ||
                idCol.type === 'uuid')
            ) {
              console.log(
                `[Tenancy] Patching 'hr_tax_brackets' table in ${targetDbName}: Dropping due to UUID type conflict`,
              );
          ...
          console.warn(
            `[Tenancy] Failed to patch tables in ${targetDbName}: ${patchErr.message}`,
          );
        }
        ...
        if (error.code === '3D000' && tenantOptions.type === 'postgres') {
          console.log(
            `[Tenancy] Database ${targetDbName} missing. Creating...`,
          );
          await this.createDatabase(tenantOptions, targetDbName);
          await tenantDS.initialize();
          console.log(
            `[Tenancy] Connection established after creation for ${targetDbName}`,
          );
        } else {
          console.error(
            `[Tenancy] Fatal error initializing ${targetDbName}:`,
            error.message,
          );
          throw error;
        }
      ...
      console.error(
        `[Tenancy] Failed to initialize tenant ${companyId}:`,
        err.message,
      );
      const errorCode = err.code ? ` (${err.code})` : '';
      throw new BadRequestException(
        `Erro de Infraestrutura${errorCode}: ${err.message}`,
      );
    }
  }
  ...
  private async createDatabase(options: any, dbName: string) {
    ...
    for (const adminDb of dbCandidates) {
      ...
      try {
        ...
        console.log(
          `Connected to admin database "${adminDb}" to create "${dbName}"`,
        );
        ...
        console.warn(
          `Failed to create database via "${adminDb}":`,
          err.message,
        );
    ...
    throw new BadRequestException(
      `Falha ao criar base de dados "${dbName}": ${lastError?.message || 'Erro desconhecido'}`,
    );
  }
  ...
  private async seedTenantData(ds: DataSource, companyId: string) {
    ...
    try {
      ...
      let fiscalYear = await fiscalYearRepo.findOne({
        where: { companyId, year: yearToUse },
      });
      ...
      let series = await seriesRepo.findOne({
        where: { companyId, code: seriesCode },
      });

      if (!series) {
        console.log(`[Tenancy] Creating Series ${seriesCode}`);
        series = new Series();
        series.id = company?.seriesConfig
          ? `SERIES-${seriesCode}-${companyId}`
          : `${yearToUse}-${companyId}`;
      ...
      if (count === 0) {
        ...
        const allDefaults = [
          ...SALES_DOCUMENT_TYPES.map((t) => ({
            ...t,
            module: t.type || 'SALES',
          })),
          ...PURCHASE_DOCUMENT_TYPES.map((t) => ({
            ...t,
            module: t.type || 'PURCHASES',
          })),
          ...TREASURY_DOCUMENT_TYPES.map((t) => ({
            ...t,
            module: t.type || 'TREASURY',
          })),
          ...STOCK_DOCUMENT_TYPES.map((t) => ({
            ...t,
            module: t.type || 'STOCK',
          })),
        ];
      ...
      if (pmCount === 0) {
        ...
        console.log(
          `[Tenancy] ✅ Created ${pmDefaults.length} payment methods.`,
        );
      }
      ...
      if (taxCount === 0) {
        ...
        const taxDefaults = [
          ...
          {
            code: '16',
            description: 'IVA Taxa Normal (16%)',
            rate: 16,
            type: 'IVA',
            companyId,
            isActive: true,
          ...
          {
            code: '17',
            description: 'IVA Taxa Anterior (17%)',
            rate: 17,
            type: 'IVA',
            companyId,
            isActive: true,
          ...
          {
            code: 'BS',
            description: 'Bens em segunda mão',
            rate: 17,
            type: 'IVA',
            companyId,
            isActive: true,
