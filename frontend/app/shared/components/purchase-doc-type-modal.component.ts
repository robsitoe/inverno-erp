...
import { PURCHASE_DOCUMENT_TYPES } from '../constants';

@Component({
    selector: 'app-purchase-doc-type-modal',
    standalone: true,
    imports: [CommonModule],
    template: `
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" (click)="onClose()">
      <div class="bg-white rounded-sm shadow-lg w-[500px] max-h-[80vh] flex flex-col" (click)="$event.stopPropagation()">
        <!-- Header -->
        <div class="flex items-center justify-between px-3 py-2 border-b border-gray-200 bg-gray-50">
          <h3 class="text-sm font-medium text-gray-700">Tipo de Documento de Compra</h3>
          ...
          <table class="w-full text-xs">
            ...
            <tbody>
              <tr *ngFor="let docType of documentTypes" 
                ...
                <td class="px-2 py-1.5 border-b border-gray-100 font-medium text-blue-600">{{ docType.code }}</td>
                <td class="px-2 py-1.5 border-b border-gray-100 text-gray-700">{{ docType.description }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class PurchaseDocTypeModalComponent {
