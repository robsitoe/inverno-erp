    ...
    <ng-container *ngIf="currentDoc; else loading">
      ...
      <div class="flex flex-col h-full w-full bg-[#F0F0F0] text-xs overflow-hidden relative">
        ...
        <div class="flex items-end px-1 pt-2 border-b border-gray-300 bg-[#E0E0E0] shrink-0 gap-1 overflow-x-auto">
          ...
          <ng-container *ngFor="let tab of tabs; let i = index"><button *ngIf="implementedTabs.includes(i)"
        ...
        <div class="bg-[#F0F0F0] border-b border-gray-300 shrink-0 h-64 overflow-y-auto relative">
          ...
          <div *ngIf="activeTab === 4" class="flex flex-col gap-2 p-4">
            <label class="text-xs font-semibold text-gray-600 uppercase tracking-wider">Observações do Documento</label>
            <textarea [(ngModel)]="currentDoc.notes" rows="8" placeholder="Notas internas ou a imprimir no documento..." class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600 resize-none"></textarea>
          </div>

          <div *ngIf="false" class="flex items-center justify-center p-8 text-gray-400">
