...
@Component({
  selector: 'app-purchase-document-search-modal',
  standalone: true,
  ...
  template: `
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow-lg w-[800px] h-[500px] flex flex-col text-xs">
        <!-- Header -->
        <div class="bg-green-600 text-white px-3 py-2 flex justify-between items-center shrink-0">
          <span class="font-bold">Procurar Documentos de Compra</span>
          <button (click)="close.emit()" class="hover:bg-green-700 rounded p-1">
        ...
        <div class="p-2 bg-gray-100 border-b border-gray-300 flex gap-2 shrink-0">
          <input 
            ...
            placeholder="Filtrar por fornecedor, número ou total..." 
            class="flex-1 border border-gray-300 rounded px-2 py-1 focus:outline-none focus:border-green-500"
          />
        </div>
          ...
          <table class="w-full border-collapse">
            <thead class="bg-gray-50 sticky top-0 shadow-sm">
              <tr>
                ...
                <th class="border-b border-r border-gray-300 px-2 py-1 text-left">Fornecedor</th>
            ...
            <tbody>
              <tr *ngFor="let doc of filteredDocuments" 
                  ...
                  class="hover:bg-green-50 cursor-pointer border-b border-gray-100">
                <td class="px-2 py-1 border-r border-gray-100">{{ doc.type }}</td>
                <td class="px-2 py-1 border-r border-gray-100">{{ doc.series }}</td>
                <td class="px-2 py-1 border-r border-gray-100 text-right">{{ doc.number }}</td>
                <td class="px-2 py-1 border-r border-gray-100">{{ doc.date | date:'dd/MM/yyyy' }}</td>
                <td class="px-2 py-1 border-r border-gray-100">{{ doc.supplierName }}</td>
                <td class="px-2 py-1 text-right font-mono">{{ doc.totalValue | number:'1.2-2' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class PurchaseDocumentSearchModalComponent implements OnInit {
  ...
  loadDocuments() {
    ...
    this.dataService.getPurchaseDocuments(companyId).subscribe(docs => {
      this.documents = docs;
      // Sort by date desc
  ...
  filterDocuments() {
    ...
    this.filteredDocuments = this.documents.filter(doc =>
      ...
      doc.number?.toString().includes(term) ||
      doc.totalValue?.toString().includes(term)
    );
  }
