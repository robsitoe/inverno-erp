    ...
    <div *ngIf="showUserModal" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50 backdrop-blur-[2px]">
      <div class="bg-[#F0F0F0] rounded-lg shadow-2xl w-[800px] h-[600px] flex flex-col overflow-hidden border border-gray-400 animate-fade-in font-sans text-sm">
        ...
        <div class="flex-1 p-6 overflow-y-auto bg-[#F0F0F0]">
          ...
          <!-- Tab: Perfis & Permissões (matriz configurável) -->
          <div *ngIf="activeTab === 'modules'" class="h-full flex gap-3 overflow-hidden">
            <div class="w-48 shrink-0 flex flex-col border border-gray-300 rounded bg-white overflow-hidden">
              <div class="px-2 py-1.5 bg-gray-50 border-b text-xs font-bold text-gray-600 flex justify-between items-center">
                Perfis
                <button (click)="newProfile()" class="text-blue-600 hover:text-blue-800" title="Novo perfil"><span class="material-symbols-outlined text-[16px]">add_circle</span></button>
              </div>
              <div class="flex-1 overflow-y-auto">
                <div *ngFor="let p of availableProfiles" (click)="selectProfile(p)"
                  [class.bg-blue-100]="selectedProfile?.id === p.id"
                  class="px-2 py-1.5 text-xs cursor-pointer hover:bg-gray-50 border-b border-gray-100 flex justify-between">
                  <span class="font-medium">{{ p.name }}</span>
                  <span class="text-gray-400">{{ (p.permissions || []).length }}</span>
                </div>
              </div>
            </div>
            <div class="flex-1 flex flex-col border border-gray-300 rounded bg-white overflow-hidden">
              <div *ngIf="!selectedProfile" class="flex-1 flex items-center justify-center text-gray-400 italic text-sm">
                Selecione um perfil à esquerda ou crie um novo.
              </div>
              <ng-container *ngIf="selectedProfile">
                <div class="px-3 py-2 bg-gray-50 border-b flex items-center gap-2">
                  <input [(ngModel)]="selectedProfile.name" class="border border-gray-300 rounded px-2 py-1 text-sm font-bold w-48">
                  <span class="text-[10px] text-gray-400">{{ (selectedProfile.permissions || []).length }} permissões</span>
                  <div class="ml-auto flex gap-2">
                    <button (click)="saveProfile()" [disabled]="savingProfile" class="px-3 py-1 bg-blue-600 text-white rounded text-xs font-bold hover:bg-blue-700 disabled:opacity-50">{{ savingProfile ? 'A guardar...' : 'Guardar Perfil' }}</button>
                    <button *ngIf="selectedProfile.id && !selectedProfile.isSystem" (click)="deleteProfile()" class="px-3 py-1 border border-red-300 text-red-600 rounded text-xs font-bold hover:bg-red-50">Eliminar</button>
                  </div>
                </div>
                <div class="flex-1 overflow-y-auto p-3 space-y-3">
                  <div *ngFor="let mod of permsCatalog" class="border border-gray-200 rounded">
                    <div class="px-2 py-1 bg-gray-50 border-b text-[11px] font-bold text-gray-600 uppercase">{{ mod.label }}</div>
                    <div class="p-2 grid grid-cols-2 gap-1">
                      <label *ngFor="let perm of mod.permissions" class="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-50 rounded px-1 py-0.5">
                        <input type="checkbox" [checked]="hasPermKey(perm.key)" (change)="togglePermKey(perm.key)" class="accent-blue-600">
                        <span>{{ perm.label }}</span>
                        <span class="text-[9px] text-gray-300 ml-auto font-mono">{{ perm.key }}</span>
                      </label>
                    </div>
                  </div>
                </div>
...
export class AdminUsersComponent implements OnInit {
  ...
  tabs = [
    ...
    { id: 'modules', label: 'Perfis & Permissões' },
    { id: 'mappings', label: 'Mapeamentos' }
  ];

  availableProfiles: any[] = [];
  permsCatalog = PERMISSIONS_CATALOG;
  ...
  private get activeCompanyId(): string {
    try { return JSON.parse(localStorage.getItem('erp_company_info') || '{}').id || ''; } catch { return ''; }
  }

  loadProfiles() {
    const cid = this.activeCompanyId;
    if (!cid) return;
    this.http.get<any[]>(environment.apiUrl + '/profiles?companyId=' + cid).subscribe({
      next: (profiles) => {
        this.availableProfiles = (profiles || []).map(p => ({ ...p, description: (p.permissions || []).length + ' permissões' }));
        if (this.selectedProfile) {
          this.selectedProfile = this.availableProfiles.find(p => p.id === this.selectedProfile.id) || null;
        }
      },
      error: (e) => console.error('Erro ao carregar perfis:', e),
    });
  }

  selectProfile(p: any) {
    this.selectedProfile = { ...p, permissions: [...(p.permissions || [])] };
  }

  hasPermKey(key: string): boolean {
    return !!this.selectedProfile && (this.selectedProfile.permissions || []).includes(key);
  }

  togglePermKey(key: string) {
    if (!this.selectedProfile) return;
    const perms: string[] = this.selectedProfile.permissions || [];
    const i = perms.indexOf(key);
    if (i >= 0) perms.splice(i, 1); else perms.push(key);
    this.selectedProfile.permissions = perms;
  }

  newProfile() {
    const name = prompt('Nome do novo perfil:');
    if (!name || !name.trim()) return;
    this.selectedProfile = { id: null, name: name.trim().toUpperCase(), permissions: [], companyId: this.activeCompanyId };
  }

  saveProfile() {
    if (!this.selectedProfile || this.savingProfile) return;
    this.savingProfile = true;
    const cid = this.activeCompanyId;
    const body = { companyId: cid, name: this.selectedProfile.name, permissions: this.selectedProfile.permissions };
    const req = this.selectedProfile.id
      ? this.http.patch(environment.apiUrl + '/profiles/' + this.selectedProfile.id, body)
      : this.http.post(environment.apiUrl + '/profiles', body);
    req.subscribe({
      next: () => { this.savingProfile = false; this.loadProfiles(); alert('Perfil guardado.'); },
      error: (e) => { this.savingProfile = false; alert('Erro: ' + (e?.error?.message || e.message)); },
    });
  }

  deleteProfile() {
    if (!this.selectedProfile || !this.selectedProfile.id) return;
    if (!confirm('Eliminar o perfil "' + this.selectedProfile.name + '"?')) return;
    this.http.delete(environment.apiUrl + '/profiles/' + this.selectedProfile.id).subscribe({
      next: () => { this.selectedProfile = null; this.loadProfiles(); },
