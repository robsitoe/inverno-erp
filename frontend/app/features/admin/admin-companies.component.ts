  ...
  template: `
    ...
    <div *ngIf="showWizard" class="fixed inset-0 bg-black/20 flex items-center justify-center z-50 backdrop-blur-[2px]">
      <div class="bg-[#F0F0F0] rounded-lg shadow-2xl w-[700px] flex flex-col overflow-hidden border border-gray-400 animate-fade-in font-sans text-sm">
          ...
          <div *ngIf="currentStep === 4" class="space-y-4 animate-slide-in">
            <div class="bg-white border border-gray-300 rounded p-6">
              ...
              <div class="grid grid-cols-2 gap-y-4 text-sm">
                ...
                <div class="font-medium">{{ seriesConfig.code }} ({{ seriesConfig.startDate | date:'dd/MM/yyyy' }} a {{ seriesConfig.endDate | date:'dd/MM/yyyy' }})</div>
              </div>
            </div>
        ...
        <!-- Wizard Footer -->
        <div class="bg-[#F0F0F0] px-6 py-3 border-t border-gray-300 flex justify-end gap-2">
          <button 
            (click)="prevStep()" 
...
export class AdminCompaniesComponent implements OnInit {
  ...
  getStepTitle() {
    switch (this.currentStep) {
      case 1: return 'Tipo de Empresa';
      case 2: return 'Tipo de Exercício e Localização da Sede';
      case 3: return 'Configuração da Série';
  ...
  getStepDescription() {
    switch (this.currentStep) {
      case 1: return 'Selecione, por favor, o tipo de empresa que pretende criar.';
      case 2: return 'Selecione, por favor, o tipo de exercício e a localização da sede.';
      case 3: return 'Defina a série inicial para os documentos e o seu período de vigência.';
      case 4: return 'Verifique os dados antes de proceder à criação da empresa.';
      default: return '';
    }
  }
  ...
  finishWizard() {
    ...
    this.dataService.saveCompanyInfo(this.newCompany).subscribe(() => {
      // Set as active company context (Local only for now as session concept isn't fully backend yet)
      localStorage.setItem('erp_company_info', JSON.stringify(this.newCompany));
    ...
    modules.forEach(mod => {
      ...
      if (stored) {
        ...
        docTypes.forEach((dt: any) => {
          ...
          // Add the configured series if it doesn't exist for this company
          if (!dt.series.find((s: any) => s.code === seriesCode && s.companyId === this.newCompany.id)) {
            dt.series.unshift({
              code: seriesCode,
