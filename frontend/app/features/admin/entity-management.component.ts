          ...
          <div *ngIf="selectedEntity" class="max-w-4xl">
            ...
            <div *ngIf="activeTab === 1" class="bg-white p-4 rounded shadow-sm space-y-4">
              <div class="grid grid-cols-2 gap-4">
                ...
                <!-- Tipo de Cliente (Gás) -->
                <div *ngIf="type === 'CUSTOMER'" class="col-span-2">
                  <label class="block text-xs font-medium text-gray-700 mb-1">Tipo de Cliente (Mapa de Gás)</label>
                  <select 
                    [(ngModel)]="selectedEntity.type"
                    class="w-full px-2 py-1 border border-gray-300 rounded text-xs focus:outline-none focus:border-blue-500"
                  >
...
export class EntityManagementComponent implements OnInit, OnChanges {
  ...
  loadAccounts() {
    ...
    if (this.type === 'CUSTOMER') {
      this.availableAccounts = allAccounts
        .filter(a => a.allowPosting && (a.code.startsWith('2.1') || a.code.startsWith('21')))
        .sort((a, b) => a.code.localeCompare(b.code));
    } else {
      this.availableAccounts = allAccounts
        .filter(a => a.allowPosting && (a.code.startsWith('2.2') || a.code.startsWith('22')))
        .sort((a, b) => a.code.localeCompare(b.code));
    }
  }
  ...
  saveEntity() {
    ...
    if (this.type === 'CUSTOMER') {
      if (typeof this.selectedEntity.id === 'string' && this.selectedEntity.id.startsWith('NEW_')) {
