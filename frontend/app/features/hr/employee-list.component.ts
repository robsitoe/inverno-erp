...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-col h-full bg-[#F0F0F0]">
                  ...
                  <ng-container *ngIf="activeTab === 'salary'">
                    ...
                    <div *ngIf="selectedEmployee.id" class="col-span-2 mt-4 bg-gray-100 p-3 rounded">
                       <div class="flex items-center justify-between mb-2">
                          <label class="block text-[10px] font-bold text-gray-700 uppercase italic">Gestão de Variações Salariais</label>
                          <button (click)="openVariationModal()" class="flex items-center gap-1 px-3 py-1 bg-blue-600 text-white rounded text-[10px] font-bold uppercase hover:bg-blue-700 transition-all shadow-sm">
                            <app-icon name="trending_up" [size]="14"></app-icon>
                            <span>Nova Variação</span>
                          </button>
                       </div>
                       <p class="text-[9px] text-gray-400 uppercase tracking-tighter">* Use o botão acima para registar aumentos ou alterações de forma profissional com justificativo e anexo.</p>
                    </div>
                  </ng-container>
      ...
      <!-- Salary Variation Modal -->
      <div *ngIf="showVariationModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white w-full max-w-lg rounded-lg shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
          <div class="px-6 py-4 border-b bg-gray-50 flex items-center justify-between">
            <h3 class="font-bold text-gray-800 uppercase tracking-wide">Nova Variação Salarial</h3>
            <button (click)="showVariationModal = false" class="text-gray-400 hover:text-gray-600">
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div class="col-span-1">
                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Novo Salário Base</label>
                <input type="number" [(ngModel)]="newVariation.newSalary" class="w-full px-3 py-2 border rounded text-sm bg-gray-50 focus:bg-white focus:ring-1 focus:ring-blue-500">
              </div>
              <div class="col-span-1">
                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Data de Vigor</label>
                <input type="date" [(ngModel)]="newVariation.effectiveDate" class="w-full px-3 py-2 border rounded text-sm bg-gray-50 focus:bg-white focus:ring-1 focus:ring-blue-500">
              </div>
            </div>

            <div class="grid grid-cols-2 gap-4">
              <div class="col-span-1">
                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Novo Subs. Transporte</label>
                <input type="number" [(ngModel)]="newVariation.newTransport" class="w-full px-3 py-2 border rounded text-sm bg-gray-50">
              </div>
              <div class="col-span-1">
                <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Novo Subs. Alimentação</label>
                <input type="number" [(ngModel)]="newVariation.newFood" class="w-full px-3 py-2 border rounded text-sm bg-gray-50">
              </div>
            </div>

            <div class="col-span-2">
              <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Motivo / Justificação</label>
              <textarea [(ngModel)]="newVariation.reason" rows="2" placeholder="Ex: Ajuste Anual, Promoção..."
                class="w-full px-3 py-2 border rounded text-sm bg-gray-50 focus:bg-white focus:ring-1 focus:ring-blue-500 resize-none"></textarea>
            </div>

            <div class="col-span-2">
              <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Anexar Adenda (PDF)</label>
              <div class="flex items-center gap-2">
                <button (click)="variationFileInput.click()" class="flex-1 px-4 py-2 border-2 border-dashed border-gray-300 rounded hover:border-blue-400 hover:bg-blue-50 transition-all flex items-center justify-center gap-2 group">
                  <app-icon name="upload_file" [size]="20" class="text-gray-400 group-hover:text-blue-500"></app-icon>
                  <span class="text-xs text-gray-500 group-hover:text-blue-700">{{ variationFile ? variationFile.name : 'Clique para carregar PDF...' }}</span>
                </button>
                <input #variationFileInput type="file" (change)="onVariationFileSelected($event)" accept="application/pdf" class="hidden">
                <button *ngIf="variationFile" (click)="variationFile = null" class="p-2 text-red-500 hover:bg-red-50 rounded">
                  <app-icon name="delete" [size]="18"></app-icon>
                </button>
              </div>
            </div>
          ...
          <div class="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
            <button (click)="showVariationModal = false" class="px-4 py-2 text-xs font-bold text-gray-500 uppercase hover:text-gray-700 transition-colors">Cancelar</button>
            <button (click)="submitVariation()" [disabled]="isSubmittingVariation"
              class="px-6 py-2 bg-blue-600 text-white rounded text-xs font-bold uppercase tracking-widest hover:bg-blue-700 shadow-md transition-all disabled:opacity-50">
              {{ isSubmittingVariation ? 'A processar...' : 'Confirmar Variação' }}
            </button>
          </div>
        </div>
      </div>
...
export class EmployeeListComponent implements OnInit, OnDestroy {
  ...
  isSubmittingVariation = false;
  variationFile: File | null = null;
  newVariation: any = {
    newSalary: 0,
  ...
  // ── Salary Variation Logic ────────────────────────────────────────────────

  openVariationModal() {
    if (!this.selectedEmployee) return;
    this.newVariation = {
      newSalary: this.selectedEmployee.salaryBase,
      effectiveDate: new Date().toISOString().split('T')[0],
      newTransport: this.selectedEmployee.subsidyTransport,
      newFood: this.selectedEmployee.subsidyFood,
      reason: ''
    };
    this.variationFile = null;
    this.showVariationModal = true;
  }

  onVariationFileSelected(event: any) {
    this.variationFile = event.target.files[0];
  }

  submitVariation() {
    if (!this.selectedEmployee?.id) return;
    if (this.newVariation.newSalary <= 0) {
      this.toaster.showError('Erro', 'O novo salário deve ser superior a zero.');
      return;
    }

    this.isSubmittingVariation = true;
    const cid = this.getCompanyId();

    const payload = {
      employeeId: this.selectedEmployee.id,
      companyId: cid,
      oldSalary: this.selectedEmployee.salaryBase,
      newSalary: this.newVariation.newSalary,
      oldTransport: this.selectedEmployee.subsidyTransport,
      newTransport: this.newVariation.newTransport,
      oldFood: this.selectedEmployee.subsidyFood,
      newFood: this.newVariation.newFood,
      oldDependents: this.selectedEmployee.dependents,
      newDependents: this.selectedEmployee.dependents,
      reason: this.newVariation.reason,
      changeDate: new Date().toISOString().split('T')[0],
      effectiveDate: this.newVariation.effectiveDate,
      status: 'APPLIED', // For now, we apply immediately
      updatedBy: 'Utilizador' // In production, get current user
    };

    this.hrService.createSalaryVariation(payload, cid).subscribe({
      next: (variation) => {
        if (this.variationFile) {
          this.hrService.uploadDocument(this.selectedEmployee!.id, cid, this.variationFile, 'CONTRATO', `Adenda Salarial - ${this.newVariation.effectiveDate}`)
            .subscribe(() => {
              this.finishVariation();
            });
        } else {
          this.finishVariation();
        }
      ...
      error: (err) => {
        this.isSubmittingVariation = false;
        this.toaster.showError('Erro ao criar variação', err.message);
      }
    });
  }

  private finishVariation() {
    this.toaster.showSuccess('Sucesso', 'Variação salarial registada com sucesso.');
    this.isSubmittingVariation = false;
    ...
    setTimeout(() => {
      if (this.selectedEmployee) {
        ...
        if (updated) this.selectEmployee(updated);
      }
    }, 500);
  }
}
