      ...
      <div class="flex-1 overflow-auto p-6 flex flex-col gap-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full bg-blue-500"></span><p class="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Activo</p></div>
            <p class="font-mono font-black text-slate-800 text-base">{{ summary.assets | number:'1.2-2' }}</p>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full bg-rose-500"></span><p class="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Passivo</p></div>
            <p class="font-mono font-black text-slate-800 text-base">{{ summary.liabilities | number:'1.2-2' }}</p>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full bg-indigo-500"></span><p class="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Capital Próprio</p></div>
            <p class="font-mono font-black text-slate-800 text-base">{{ summary.equity | number:'1.2-2' }}</p>
          </div>
          <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
            <div class="flex items-center gap-2 mb-1"><span class="w-2 h-2 rounded-full" [class.bg-emerald-500]="summary.netResult >= 0" [class.bg-amber-500]="summary.netResult < 0"></span><p class="text-[10px] uppercase tracking-widest text-slate-400 font-bold">Resultado Líquido</p></div>
            <p class="font-mono font-black text-base" [class.text-emerald-700]="summary.netResult >= 0" [class.text-amber-600]="summary.netResult < 0">{{ summary.netResult | number:'1.2-2' }}</p>
            <p class="text-[9px] text-slate-400 mt-0.5">{{ summary.netResult >= 0 ? 'Lucro' : 'Prejuízo' }} (Rend. − Gastos)</p>
          </div>
          <div class="rounded-2xl shadow-sm p-4 border" [class.bg-emerald-50]="summary.equationOk" [class.border-emerald-200]="summary.equationOk" [class.bg-rose-50]="!summary.equationOk" [class.border-rose-200]="!summary.equationOk">
            <div class="flex items-center gap-2 mb-1"><span class="material-symbols-outlined text-sm" [class.text-emerald-600]="summary.equationOk" [class.text-rose-600]="!summary.equationOk">{{ summary.equationOk ? 'balance' : 'warning' }}</span><p class="text-[10px] uppercase tracking-widest font-bold" [class.text-emerald-600]="summary.equationOk" [class.text-rose-600]="!summary.equationOk">Equação</p></div>
            <p class="font-mono font-black text-base" [class.text-emerald-700]="summary.equationOk" [class.text-rose-700]="!summary.equationOk">{{ summary.equationOk ? 'OK' : (summary.equationDiff | number:'1.2-2') }}</p>
            <p class="text-[9px] text-slate-400 mt-0.5">Activo = Pas. + Cap. + Result.</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 flex flex-wrap items-center gap-3">
          <div class="relative flex-1 min-w-[220px]">
            ...
            <input [(ngModel)]="searchTerm" placeholder="Pesquisar por código ou nome da conta..." class="w-full pl-10 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent">
          </div>
          <div class="flex gap-1 flex-wrap">
            <button (click)="typeFilter = ''" [class]="chipClass('')">Todas</button>
            <button (click)="typeFilter = 'ASSET'" [class]="chipClass('ASSET')">Activo</button>
            <button (click)="typeFilter = 'LIABILITY'" [class]="chipClass('LIABILITY')">Passivo</button>
          ...
          <span class="text-[11px] text-slate-400 font-medium">{{ displayedRows.length }} conta(s)</span>
        </div>
          ...
          <div class="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <table class="w-full text-sm border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                  ...
                  <tr *ngFor="let item of displayedRows; trackBy: trackByAccount"
...
export class TrialBalanceComponent implements OnInit {
  ...
  viewIssueDetails(issue: AccountingIssue) {
    ...
    if (issue.type === 'MISSING_POSTING' && issue.relatedId) {
      ...
      if (issue.details && issue.details['docType'] === 'TREASURY') {
        this.dataService.getTreasuryDocuments().subscribe(docs => {
          const doc = docs.find((d: any) => d.id === issue.relatedId);
          if (doc) {
            try { this.selectedIssueEntry = this.accountingService.createTreasuryJournalEntry(doc, true) || undefined; } catch (e) { console.error('Preview treasury failed', e); }
            this.cdr.detectChanges();
          }
        });
      } else if (issue.details && issue.details['docType'] === 'SALES') {
...
<body>
  ...
  onGoToDocument(issue: AccountingIssue) {
    ...
    const dt = issue.details['docType']; const view = dt === 'SALES' ? 'sales-form' : (dt === 'TREASURY' ? 'treasury-management' : 'purchase-form');
  ...
  fixSingleMissingPosting(issue: AccountingIssue) {
    ...
    this.isRepairing = true;
    const docType = issue.details && issue.details['docType'];
    const done = () => {
      // Give the async save a tick, then recalculate & persist balances so the
      // new posting is reflected in the trial balance (which reads account.balance).
      setTimeout(() => {
        this.accountingService.recalculateAllBalances().subscribe({
          next: () => { this.isRepairing = false; this.loadTrialBalance(); this.cdr.detectChanges(); },
          error: () => { this.isRepairing = false; this.loadTrialBalance(); this.cdr.detectChanges(); },
        });
      }, 400);
    };
    const fail = (msg: string) => { this.isRepairing = false; (this.accountingService as any)['toasterService']?.showError('Falha ao Lançar', msg); this.cdr.detectChanges(); };

    if (docType === 'SALES') {
      this.dataService.getSalesDocuments().subscribe(docs => {
        const doc = docs.find((d: any) => d.id === issue.relatedId);
        if (doc) { this.accountingService.createSalesJournalEntry(doc, null, []); done(); }
        else fail('Documento de venda não encontrado.');
      });
    } else if (docType === 'TREASURY') {
      this.dataService.getTreasuryDocuments().subscribe(docs => {
        const doc = docs.find((d: any) => d.id === issue.relatedId);
        if (!doc) { fail('Documento de tesouraria não encontrado.'); return; }
        const res = this.accountingService.createTreasuryJournalEntry(doc);
        if (!res) fail('Não foi possível lançar (valor inválido ou tipo não integrável).');
        else done();
      });
    } else {
      this.dataService.getPurchaseDocuments().subscribe(docs => {
        const doc = docs.find((d: any) => d.id === issue.relatedId);
        if (doc) { this.accountingService.createPurchaseJournalEntry(doc, null); done(); }
        else fail('Documento de compra não encontrado.');
      });
    }
  }
  ...
  /** Net balance per class using root accounts only (avoids double counting). */
  private classNet(type: string): number {
    const ids = new Set(this.trialBalance.map(i => i.account.id));
    const roots = this.trialBalance.filter(i =>
      i.account.type === type && (!i.account.parentId || !ids.has(i.account.parentId))
    );
    const debit = roots.reduce((s, i) => s + (+i.debit || 0), 0);
    const credit = roots.reduce((s, i) => s + (+i.credit || 0), 0);
    // Assets & Expenses are debit-natured; the rest are credit-natured
    return (type === 'ASSET' || type === 'EXPENSE') ? debit - credit : credit - debit;
  }

  get summary() {
    const assets = this.classNet('ASSET');
    const liabilities = this.classNet('LIABILITY');
    const equity = this.classNet('EQUITY');
    const revenue = this.classNet('REVENUE');
    const expense = this.classNet('EXPENSE');
    const netResult = revenue - expense;
    const equationDiff = assets - (liabilities + equity + netResult);
    return {
      assets, liabilities, equity, revenue, expense, netResult,
      equationDiff,
      equationOk: Math.abs(equationDiff) < 0.01,
    };
  }
