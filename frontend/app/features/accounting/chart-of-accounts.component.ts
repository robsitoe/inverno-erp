...
import { ChartOfAccountsSetupComponent } from './chart-of-accounts-setup.component';
import { AccountPlanManagerComponent } from './account-plan-manager.component';
...
@Component({
  ...
  imports: [CommonModule, FormsModule, ChartOfAccountsSetupComponent, AccountPlanManagerComponent],
  ...
  template: `
    ...
    <!-- Account Plan Manager Modal -->
    <app-account-plan-manager *ngIf="showPlanManager" (closed)="onPlanManagerClose($event)"></app-account-plan-manager>

    <!-- Setup Banner: shown when no accounts exist -->
    <div *ngIf="accounts.length === 0 && !setupDismissed" class="p-4 shrink-0">
          ...
          <tbody>
            ...
            <tr *ngFor="let account of accounts" class="hover:bg-gray-50">
              ...
              <td class="p-2 border text-right font-mono" [class.font-bold]="!account.allowPosting">
                ...
                {{ (+account.balance || 0) | number:'1.2-2' }} MT
...
export class ChartOfAccountsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (stored) { const c = JSON.parse(stored); this.companyName = c.name || ''; this.companyId = c.id || ''; }
  ...
  onPlanManagerClose(changed: boolean) {
    this.showPlanManager = false;
    if (changed) { setTimeout(() => this.loadAccounts(), 300); }
  }

  onSetupComplete(count: number) {
