...
@Component({
  ...
  template: `
    ...
    <div class="flex flex-col h-full w-full bg-[#F0F0F0] text-xs overflow-hidden relative">
      ...
      <div class="bg-white border-b border-gray-200 shrink-0 h-64 overflow-y-auto relative">

        <!-- Locked Overlay -->

        <div *ngIf="isLocked" class="absolute inset-0 bg-white/60 z-20 flex items-center justify-center pointer-events-none backdrop-blur-[1px]">

          <div class="bg-red-600 text-white px-5 py-2.5 rounded-xl shadow-2xl font-black text-sm uppercase tracking-widest transform -rotate-6 border-2 border-white">
     ...
     <div class="flex-1 overflow-auto bg-white border-t border-gray-300 relative">
       ...
       <table class="w-full border-collapse table-fixed">
          ...
          <thead class="bg-gray-50 sticky top-0 z-10 shadow-sm text-[10px] uppercase">
            ...
            <tr>
              ...
              <th class="px-2 py-1 border-r border-b border-l border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[120px]">Artigo</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[60px]">Arm.</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">Localização</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">Lote</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap min-w-[300px]">Descrição</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[50px]">CIVA</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[50px]">IVA %</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[90px]">Pr. Unit.</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[60px]">Desc.</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[40px]">UN</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[70px]">Qtd.</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[90px]">Total Liq.</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">Projeto</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">Elem. PEP</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[100px]">Cód. Barras</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[120px]">IVA - Regra Cálculo</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[90px]">Valor Total</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">Contrato</th>

              <th class="px-2 py-1 border-r border-b border-gray-200 bg-slate-700 text-white font-semibold text-left whitespace-nowrap w-[80px]">N.º Processo</th>
      ...
      <!-- Workflow Notes Modal -->
      <div *ngIf="showWorkflowNotesModal" class="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" (click)="cancelWorkflowAction()">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md" (click)="$event.stopPropagation()">
          <div class="flex items-center gap-3 p-5 border-b border-gray-100">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center"
                 [ngClass]="{
                   'bg-blue-100': pendingWorkflowAction === 'SUBMIT',
                   'bg-green-100': pendingWorkflowAction === 'APPROVE',
                   'bg-red-100': pendingWorkflowAction === 'REJECT',
                   'bg-purple-100': pendingWorkflowAction === 'POST'
                 }">
              <span class="material-symbols-outlined text-[20px]"
                    [ngClass]="{
                      'text-blue-600': pendingWorkflowAction === 'SUBMIT',
                      'text-green-600': pendingWorkflowAction === 'APPROVE',
                      'text-red-600': pendingWorkflowAction === 'REJECT',
                      'text-purple-600': pendingWorkflowAction === 'POST'
                    }">
                {{ pendingWorkflowAction === 'SUBMIT' ? 'send' : pendingWorkflowAction === 'APPROVE' ? 'check_circle' : pendingWorkflowAction === 'REJECT' ? 'cancel' : 'account_balance' }}
              </span>
            </div>
            <div>
              <h3 class="font-bold text-gray-900 text-sm">{{ getWorkflowActionLabel() }} Documento</h3>
              <p class="text-xs text-gray-500">{{ selectedDocType }}/{{ currentSeries }}-{{ currentSeriesNumber }}</p>
            </div>
          </div>
          <div class="p-5">
            <label class="block text-xs font-semibold text-gray-700 mb-1.5">Notas / Justificação <span class="font-normal text-gray-400">(opcional)</span></label>
            <textarea [(ngModel)]="workflowNotes" rows="3" autofocus
              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
              placeholder="Adicione notas ou justificação para esta acção..."></textarea>
          </div>
          <div class="flex gap-2 px-5 pb-5">
            <button (click)="cancelWorkflowAction()"
              class="flex-1 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors">
              Cancelar
            </button>
            <button (click)="confirmWorkflowAction()"
              class="flex-1 py-2.5 text-sm font-bold text-white rounded-xl transition-colors"
              [ngClass]="{
                'bg-blue-600 hover:bg-blue-700': pendingWorkflowAction === 'SUBMIT',
                'bg-green-600 hover:bg-green-700': pendingWorkflowAction === 'APPROVE',
                'bg-red-600 hover:bg-red-700': pendingWorkflowAction === 'REJECT',
                'bg-purple-600 hover:bg-purple-700': pendingWorkflowAction === 'POST'
              }">
...
export class SalesDocumentFormComponent implements OnDestroy {
  ...
  // Workflow Notes Modal
  showWorkflowNotesModal = false;
  workflowNotes = '';
  pendingWorkflowAction: 'SUBMIT' | 'APPROVE' | 'REJECT' | 'POST' | null = null;
  ...
  onEntitySelect(entity: any) {
    ...
    if (this.rows.some(r => r.articleCode)) {
      if (true) { // Auto-update prices on customer change
  ...
  onArticleBlur(index: number) {
    ...
    if (article) {
      ...
      // Article not in cache — try backend API
      this.dataService.getArticles().subscribe({
        next: (articles: any[]) => {
          const found = articles?.find((a: any) => a.code?.toLowerCase() === code.toLowerCase());
          if (found) {
            this.updateRowWithArticle(index, found);
          } else {
            this.toaster.showWarning('Artigo Inválido', `O código de artigo '\' não foi encontrado.`);
            this.rows[index].articleCode = '';
          }
        },
        error: () => {}
      });
  ...
  private updateRowWithArticle(index: number, article: any) {
    ...
    this.rows[index] = {
      ...
      unitPrice: this.getArticlePrice(article),

      iva: (parseFloat(String(article.ivaRate)) || 0) + '%',
    ...
    // Default IVA mapping
    const ivaRate = parseFloat(String(article.ivaRate)) || 0;
    if (ivaRate >= 23) {
      this.rows[index].civa = "23";
    } else if (ivaRate > 0) {
      this.rows[index].civa = article.ivaCode || "02";
    } else {
  ...
  /** Convert any price value (string|number) from DB to number safely */
  ...
  /** Get correct sale price based on customer type */
  private getArticlePrice(article: any): number {
    const customerType = this.selectedCustomer?.type;
    if (customerType === 'REVENDEDOR' && this.toNum(article.priceReseller) > 0) {
      return this.toNum(article.priceReseller);
    }
    if (customerType === 'BOMBA' && this.toNum(article.pricePump) > 0) {
      return this.toNum(article.pricePump);
    }
    if (customerType === 'CONSUMIDOR' && this.toNum(article.priceFinal) > 0) {
      return this.toNum(article.priceFinal);
    }
    return this.toNum(article.salePrice);
  }
  ...
  duplicateDocument() {
    ...
    this.toaster.showSuccess('Documento Duplicado', 'Novo documento criado a partir do duplicado. Verifique os dados e grave.');
  ...
  voidAndDuplicate() {
    ...
    if (!this.currentId) {
      ...
      this.toaster.showWarning('Accao Invalida', 'Grave o documento antes de o anular.');

      return;

    }

    if (true) { // Cancel and duplicate
  ...
  saveDocument(isPrinting: boolean = false, isCancelling: boolean = false, confirmedJournalEntry?: any) {
    ...
    if (this.isLocked && !isPrinting && !isCancelling) {
      ...
      this.toaster.showWarning('Documento Bloqueado', 'Este documento esta bloqueado e nao pode ser alterado.');
    ...
    if (!this.periodService.isPeriodOpen(this.documentDate)) {
      ...
      this.toaster.showError('Periodo Fechado', 'O periodo para esta data esta fechado. Nao e possivel gravar documentos nesta data.');
    ...
    if (series && series.startDate && series.endDate) {
      ...
      if (docDate < start || docDate > end) {
        ...
        this.toaster.showWarning('Serie Invalida', `Atenção: a data do documento esta fora do intervalo da serie ${series.code}.`);
    ...
    if (!this.selectedDocType) {
      ...
      this.toaster.showWarning('Campo Obrigatorio', 'Por favor selecione um tipo de documento.');
    ...
    if (!this.selectedEntityCode) {
      ...
      this.toaster.showWarning('Campo Obrigatorio', 'Por favor selecione uma entidade.');
    ...
    if (!this.currentSeries) {
      ...
      this.toaster.showWarning('Campo Obrigatorio', 'A serie do documento e obrigatoria.');
    ...
    if (!this.currentSeriesNumber) {
      ...
      this.toaster.showWarning('Campo Obrigatorio', 'O numero do documento e obrigatorio.');
    ...
    if (validLines.length === 0) {
      ...
      this.toaster.showWarning('Sem Artigos', 'Por favor adicione pelo menos um artigo ao documento.');
      ...
      this.ngZone.run(() => {
        ...
        this.dataService.saveSalesDocument(salesDoc).subscribe({
          ...
          error: (err) => {
            ...
            this.toaster.showError('Erro ao Gravar', err.error?.message || err.message || 'Erro desconhecido ao gravar o documento.');
  ...
  processPostSaveActions(
    ...
    if (isPrinting) {
      ...
      this.toaster.showSuccess('Anulado', `Documento ${documentNumber} anulado com sucesso.`);
      ...
      try {
        ...
        this.toaster.showWarning('Aviso', 'Documento gravado mas ocorreu um erro ao processar os movimentos associados.');
  ...
  sendDocument() {
    this.toaster.showInfo('Em Breve', 'O envio de documentos por email estará disponível em breve.');
  }



  openCrm() {
    this.toaster.showInfo('Em Breve', 'A integração com CRM estará disponível em breve.');
  }



  openContext() {
    this.toaster.showInfo('Em Breve', 'As opções de contexto estarão disponíveis em breve.');
  }



  showHelp() {
    this.toaster.showInfo('Suporte', 'Para assistência técnica, contacte o suporte em suporte@inverno-erp.com');
  }
  ...
  onWorkflowAction(action: 'SUBMIT' | 'APPROVE' | 'REJECT' | 'POST') {
    ...
    if (!this.currentId) {
      ...
      this.toaster.showWarning('Accao Invalida', 'Grave o documento antes de processar o workflow.');
    ...
    this.workflowNotes = '';
    this.showWorkflowNotesModal = true;
  }

  cancelWorkflowAction() {
    this.showWorkflowNotesModal = false;
    this.pendingWorkflowAction = null;
    this.workflowNotes = '';
  }

  getWorkflowActionLabel(): string {
    const m: Record<string, string> = { SUBMIT: 'Submeter', APPROVE: 'Aprovar', REJECT: 'Rejeitar', POST: 'Lancar' };
    return m[this.pendingWorkflowAction || ''] || 'Confirmar';
  }

  confirmWorkflowAction() {
    if (!this.pendingWorkflowAction || !this.currentId) return;
    const action = this.pendingWorkflowAction;
    const notes = this.workflowNotes;
    this.showWorkflowNotesModal = false;
    this.pendingWorkflowAction = null;
    ...
    this.dataService.processWorkflow('sales', this.currentId, action, notes).subscribe({
      ...
      next: (res) => {
        ...
        this.toaster.showSuccess('Workflow Actualizado', action === 'SUBMIT' ? 'Documento submetido para aprovação.' : action === 'APPROVE' ? 'Documento aprovado com sucesso.' : action === 'REJECT' ? 'Documento rejeitado.' : 'Documento lançado na contabilidade.');

      },

      error: (err) => {

        this.toaster.showError('Erro no Workflow', err.error?.message || err.message || 'Erro desconhecido.');
  ...
  validateSequentialDate(): boolean {
    ...
    if (lastDoc) {
      ...
      if (current < last) {
        this.toaster.showWarning('Aviso de Data', `A data (\) e anterior ao ultimo documento emitido nesta serie. Verifique a sequencia cronologica.`);
        return true;
      }
  ...
  isStepCompleted(stepStatus: string): boolean {
    ...
    const currentIdx = order.indexOf(this.status);
    const stepIdx = order.indexOf(stepStatus);
    return stepIdx < currentIdx;
  }


  getFilledLinesCount(): number {
    return this.rows.filter(r => r.articleCode && r.articleCode.trim() !== '').length;
  }
}
